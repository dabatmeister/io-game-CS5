...
monster.prototype.findPlayerToChase = function(){
  ...
  // with a value of 0
  for(distancesCounter = 0; distancesCounter < this.distances.length; distancesCounter++){
    this.distances[distancesCounter] = computeDistance(this.x, this.y, players[keys[distancesCounter]].x, players[keys[distancesCounter]].y);
    if((this.distances[distancesCounter] < shortestDistance) && (players[keys[distancesCounter]].radius < this.radius)){
      shortestDistance = this.distances[distancesCounter];
      playerToChase = distancesCounter;
    }
  }
...
io.on('connection', function(socket) {
  ...
  io.to(socket.id).emit('constants', worldWidth, worldHeight, minSizeToExist, startingPlayerRadius);
...
// initialized at 0
for(obstacleCreationCounter = 0; obstacleCreationCounter < obstacles.length; obstacleCreationCounter++){
  createObstacle(obstacleCreationCounter);
}
...
function noConflicts(objects, spawningObjectRadius, xCoord, yCoord){
  ...
  for(keyCounter = 0; keyCounter < keys.length; keyCounter++){
    distance = computeDistance(objects[keys[keyCounter]].x, objects[keys[keyCounter]].y, xCoord, yCoord);
    ...
    if(distance < (objects[keys[keyCounter]].radius+spawningObjectRadius+5)){
      return false;
    }
  }
  return true;
...
function checkFood(){
  ...
  while(foodDeleteCounter < food.length){
    // keysCounter is used to access elements in the keys array; the first element in this array has an index
    // of 0, hence keysCounter starts at 0
    for(keysCounter = 0; keysCounter < keys.length; keysCounter++){
      foodDeleteCounter = tryToEat(players[keys[keysCounter]], foodDeleteCounter);
    }
...
function moveMonsters(){
  // monstersCounter starts at 0 because it is used to access elements of the monsters array, the first 
  // element of which has an index of 0
  for(monstersCounter = 0; monstersCounter < monsters.length; monstersCounter++){
    if(monsters[monstersCounter].radius < minSizeToExist){
      ...
      monsters[monstersCounter] = new monster(spawnCoordinates[0], spawnCoordinates[1]);
    }
    monsters[monstersCounter].changePosition(monsters[monstersCounter].findPlayerToChase());                       
  }
}
...
function obstaclesDamageMonstersAndPlayers(){
  // monsterCounter is used to reference elements in monsters, which is a 0-based array, hence it
  // is initialized at a value of 0
	for(monsterCounter = 0; monsterCounter < monsters.length; monsterCounter++){
    if(monsters[monsterCounter].touchingObstacle()){
       monsters[monsterCounter].changeArea(- obstacleDamage);
    }
  }
  keys = Object.keys(players);
  // keysIndex is used to reference elements in keys, which is a 0-based array, hence it
  // is initialized at a value of 0
  for(keysIndex = 0; keysIndex < keys.length; keysIndex++){
    if(players[keys[keysIndex]].touchingObstacle()){
      players[keys[keysIndex]].changeArea(- obstacleDamage);
    }
  }
}
...
function findHighestScore(){
  ...
  highestScore = 0;
  // keyIndex references elements in keys, a 0-based array, hence it is initialized at 0
  for(keyIndex = 0; keyIndex < keys.length; keyIndex++){
    playerR = players[keys[keyIndex]].radius;
