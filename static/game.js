...
// Variables have same meaning as the equivalently named ones in server.js
var worldWidth = 1800;
...
var movement = {
  ...
  playerY : 0,
  // players start with a radius of 30
  radius: 30,
  cID: clientID,
}
...
function updateStuff(objects){
    ...
    // ii is used to reference elements of keys, a 0-based array, hence it is
    // initialized at 0
    for(ii = 0; ii < keys.length; ii++){
        if(objects[keys[ii]].cID == movement.cID){
          thisPlayer = objects[keys[ii]];
      }
    }
...
setInterval(function() {
  ...
  coordUpdated = false;
  // the 40 represents how many times per second the code in setInterval() will be executed
...
socket.on('state', function(players, obstacles, food, monsters, highestScore) {
  ...
  if(movement.radius >= .02){
    context.fillStyle = "#e6e6e6";
    // the 0s represent the x and y coordinates of the top left point of this rectangle
    context.fillRect(0, 0, canvas.width, canvas.height);
    var foundSelf = false;
    keys = Object.keys(players);
    // ii is used to reference elements of keys, a 0-based array, hence it is initialized at 0
    for(ii = 0; ii < keys.length; ii++){
      if(players[keys[ii]].cID == movement.cID){
        foundSelf = true;
      }
    }
    if(foundSelf){
